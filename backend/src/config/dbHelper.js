    ...
    if (isAdmin && req?.query?.user) {
      ...
      if (req?.user?.role === 'sales' || req?.user?.role === 'customer') {
         ...
         shipmentsUnion = `(SELECT * FROM (SELECT *, ROW_NUMBER() OVER (
            PARTITION BY CASE WHEN cust_req_no IS NOT NULL AND TRIM(cust_req_no) != '' THEN (CASE WHEN CHAR_LENGTH(TRIM(cust_req_no)) - CHAR_LENGTH(REPLACE(TRIM(cust_req_no), '-', '')) >= 1 THEN SUBSTRING_INDEX(TRIM(cust_req_no), '-', 1) ELSE TRIM(cust_req_no) END) WHEN ref_no LIKE 'ARG-%-%' THEN SUBSTRING_INDEX(ref_no, '-', 2) WHEN CHAR_LENGTH(ref_no) - CHAR_LENGTH(REPLACE(ref_no, '-', '')) >= 1 THEN SUBSTRING_INDEX(ref_no, '-', 1) ELSE ref_no END 
            ORDER BY 
